...
def define_components(mod):
    ...
    # # new
    # mod.NEW_GEN_BLD_YRS_GENERATORS = Set(
    # dimen=1,
    # initialize=lambda m: unique_list(g for g, _ in m.NEW_GEN_BLD_YRS)
    # )
    # # new
    # mod.NEW_GEN_BLD_YRS_FILTRED = Set(
    # dimen=2,
    # initialize=lambda m: {
    #     (g, bld_yr)
    #     for g in m.NEW_GEN_BLD_YRS_GENERATORS  # Solo generadores no predeterminados
    #     for bld_yr in m.PERIODS
    #     if (
    #         # Para tecnologías restringidas, no permitir antes de 2029
    #         (m.gen_tech[g] == "Bomb" and bld_yr >= 2029) or
    #         # Para las demás tecnologías, permitir desde 2026
    #         (m.gen_tech[g] != "Bomb" and bld_yr >= 2026)
    #     )
    # }
    # )

    # mod.GEN_BLD_YRS2 = Set(
    #     dimen=2, initialize=lambda m: m.NEW_GEN_BLD_YRS_FILTRED + m.PREDETERMINED_GEN_BLD_YRS
    # )
...
def load_inputs(mod, switch_data, inputs_dir):
    ...
    switch_data.load_aug(
        ...
        param=(
            ...
            mod.peak_demand_gen_max_percent
        ),
    )
