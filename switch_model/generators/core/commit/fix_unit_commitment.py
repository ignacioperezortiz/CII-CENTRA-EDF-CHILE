...
def define_components(mod):
    ...
    """
    mod.PREDETERMINED_UNIT_COMMITMENT_TPS = Set(
        dimen=2)
    mod.unit_commitment_predetermined = Param(
        mod.PREDETERMINED_UNIT_COMMITMENT_TPS,
    ...
    mod.gen_max_commit_fraction = Param(
        mod.PREDETERMINED_UNIT_COMMITMENT_TPS, within=PercentFraction, default=lambda m, g, t: 1.0
    )
    ...
    mod.CommitSlackUp = Expression(
        mod.PREDETERMINED_UNIT_COMMITMENT_TPS, rule=lambda m, g, t: (m.CommitUpperLimit[g, t] - m.unit_commitment_predetermined[g, t])
    )
    mod.CommitSlackDown = Expression(
        mod.PREDETERMINED_UNIT_COMMITMENT_TPS, rule=lambda m, g, t: (m.unit_commitment_predetermined[g, t] - m.CommitLowerLimit[g, t])
    )
    # StartupGenCapacity & ShutdownGenCapacity (at start of each timepoint)
    mod.PREDETERMINED_START_UP_GEN_TPS = Set(
        dimen=2)
    mod.unit_start_up_predetermined = Param(
        mod.PREDETERMINED_START_UP_GEN_TPS,
        within=NonNegativeReals)
    # mod.PREDETERMINED_SHUT_DOWN_GEN_TPS = Set(
    #     dimen=2)
    # mod.unit_shut_down_predetermined = Param(
    #     mod.PREDETERMINED_SHUT_DOWN_GEN_TPS,
    #     within=NonNegativeReals)
    mod.StartupGenCapacity = Var(mod.PREDETERMINED_UNIT_COMMITMENT_TPS, within=NonNegativeReals)
    # mod.ShutdownGenCapacity = Var(mod.PREDETERMINED_UNIT_COMMITMENT_TPS, within=NonNegativeReals)
    ...
    mod.Total_StartupGenCapacity_OM_Costs = Expression(
        mod.TIMEPOINTS,
        rule=lambda m, t: sum(
            m.gen_startup_om[g] * m.unit_start_up_predetermined[g, t] / m.tp_duration_hrs[t]
    ...
    # expresion de consistencia, no necesaria en problema ya resuelto
    ...
    #     ],
    # )
    # Restriccion de consistencia, no necesaria en problema ya resuelto
    ...
    mod.DispatchLowerLimit = Expression(
        mod.PREDETERMINED_UNIT_COMMITMENT_TPS,
        rule=lambda m, g, t: (m.unit_commitment_predetermined[g, t] * m.gen_min_load_fraction_TP[g, t]),
    )

    def DispatchUpperLimit_expr(m, g, t):
        if g in m.VARIABLE_GENS:
            return m.unit_commitment_predetermined[g, t] * m.gen_max_capacity_factor[g, t]
        else:
            return m.unit_commitment_predetermined[g, t]

    mod.DispatchUpperLimit = Expression(mod.PREDETERMINED_UNIT_COMMITMENT_TPS, rule=DispatchUpperLimit_expr)
    ...
    * gen_unit_size * gen_availability. The use of gen_availability here
    ...
    """
    ## No son necesarios, ya que las decisiones de unit commit vendran fijadas ya cumpliendo esta restriccion
...
def load_inputs(mod, switch_data, inputs_dir):
    ...
    switch_data.load_aug(
        optional=False,
        filename=os.path.join(inputs_dir, 'gen_unit_commitment_predetermined.csv'),
        index=mod.PREDETERMINED_UNIT_COMMITMENT_TPS,
        param=(mod.unit_commitment_predetermined))
    
    switch_data.load_aug(
        optional=False,
        filename=os.path.join(inputs_dir, 'gen_unit_start_up_predetermined.csv'),
        index=mod.PREDETERMINED_START_UP_GEN_TPS,
        param=(mod.unit_start_up_predetermined))
    
    # switch_data.load_aug(
    #     optional=False,
    #     filename=os.path.join(inputs_dir, 'gen_unit_shut_down_predetermined.csv'),
    #     index=mod.PREDETERMINED_SHUT_DOWN_GEN_TPS,
    #     param=(mod.unit_shut_down_predetermined))
...
#     write_table(
#         m,
#         m.PREDETERMINED_UNIT_COMMITMENT_TPS,
#         output_file=os.path.join(outdir, "unit_commitment.csv"),
#         headings=(
#             "GENERATION_PROJECT",
#             "timepoint",
#             "CommitGen",
...
#             g,
#             t,
#             m.unit_commitment_predetermined[g,t],
#         ),
#     )
