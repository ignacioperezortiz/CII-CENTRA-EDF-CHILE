...
def define_components(mod):
    ...
    mod.initial_bld_year = Param(mod.TRANSMISSION_LINES, within=NonNegativeIntegers)
    mod.expansion_year = Param(mod.TRANSMISSION_LINES, within=NonNegativeIntegers)
    mod.expansion_trans_cap = Param(mod.TRANSMISSION_LINES, within=NonNegativeReals)
    ...
    mod.TRANS_BLD_YRS = Set(
        ...
        filter=lambda m, tx, p: m.initial_bld_year[tx] <= p)
    mod.BuildTx = Var(mod.TRANS_BLD_YRS, within=NonNegativeReals)
    def VariableLineCapacity(m, tx, period):
        if m.expansion_year[tx] <= period:
            fix_capacity = m.existing_trans_cap[tx] + m.expansion_trans_cap[tx]
        else:
            fix_capacity = m.existing_trans_cap[tx]
        return sum(
            m.BuildTx[tx, bld_yr]
...
def load_inputs(mod, switch_data, inputs_dir):
    ...
    switch_data.load_aug(
        ...
        param=(
            ...
            mod.initial_bld_year, 
            mod.expansion_year, 
            mod.expansion_trans_cap
        ),
    )
    switch_data.load_aug(
