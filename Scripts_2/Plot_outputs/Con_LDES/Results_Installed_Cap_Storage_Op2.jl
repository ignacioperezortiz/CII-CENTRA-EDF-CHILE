...
for scenario in scenarios
    ...
    for year in missing_years_energy
        for tech in unique(pivot_df_energy.gen_tech)
            push!(pivot_df_energy, (year, tech, 0.0))
        end
    end

    for year in missing_years_capacity
        for tech in unique(pivot_df_capacity.gen_tech)
            push!(pivot_df_capacity, (year, tech, 0.0))
        end
    end
    ...
    ax_energy = Axis(fig_energy[1, 1],
        title="Capacidad de Almacenamiento (GWh) por Año (Scenario: $(basename(scenario)))", # GWh
        xlabel="Periodo",
    ...
    ax_capacity = Axis(fig_capacity[1, 1],
        title="Capacidad Instalada (GW) por Año (Scenario: $(basename(scenario)))", # GW
        xlabel="Periodo",
        ylabel="Capacidad Instalada (GW)", # GW
        xticks=(1:length(periods), string.(periods)),
        # Establecer el límite del eje y en 15 para la capacidad
        limits=(nothing, nothing, nothing, 15),
        titlesize=24,
        xlabelsize=16,
        ylabelsize=16)
    ...
    unique_techs_energy = unique(pivot_df_energy.gen_tech)
    unique_techs_capacity = unique(pivot_df_capacity.gen_tech)
    ...
    pivot_df_energy[:, :tech_position] = [tech_position_mapping_energy[tech] for tech in pivot_df_energy.gen_tech]
    pivot_df_capacity[:, :tech_position] = [tech_position_mapping_capacity[tech] for tech in pivot_df_capacity.gen_tech]
    ...
    barplot_energy = barplot!(ax_energy,
        ...
        stack=pivot_df_energy.tech_position, # Use the integer positions for stacking
        color=[colors[tech] for tech in pivot_df_energy.gen_tech],
        width=0.5,
    ...
    barplot_capacity = barplot!(ax_capacity,
        ...
        stack=pivot_df_capacity.tech_position, # Use the integer positions for stacking
        color=[colors[tech] for tech in pivot_df_capacity.gen_tech],
        width=0.5,
    ...
    # Guardar las figuras
    save(joinpath(scenario, "storage_annual_bar_chart_energy.png"), fig_energy)
    save(joinpath(scenario, "storage_annual_bar_chart_capacity.png"), fig_capacity)
