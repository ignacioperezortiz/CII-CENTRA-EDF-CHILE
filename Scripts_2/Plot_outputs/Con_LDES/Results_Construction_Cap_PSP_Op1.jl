...
for scenario in scenarios
    ...
    # Calcular Storage_Energy_GWh para cada fila (cambiar MWh a GWh)
    ...
    # Crear tablas pivotadas para Storage_Energy_GWh y GenCapacity_GW, sin sumar valores para duplicados y agrupando por duración de almacenamiento
    ...
    # Convertir GenCapacity_MW a GW
    for col in names(pivot_df_capacity)[2:end]
    ...
    CSV.write(joinpath(scenario, "bomb_storage_capacity_gw.csv"), pivot_df_capacity2) #cambiado el nombre del archivo
    ...
    # Crear la figura y el eje con proporciones ajustadas para Storage_Energy_GWh
    fig_energy = Figure(size=(1000, 600))
    ax_energy = Axis(fig_energy[1, 1], title="Capacidad construida de Almacenamiento PSP (GWh) por Duración (Escenario: $(basename(scenario)))", xlabel="Periodo", ylabel="Capacidad de Almacenamiento (GWh)",
    ...
    ylims!(ax_energy, 0, 25) # Establecer el límite del eje Y para GWh

    display(fig_energy)

    # Guardar la figura en la ruta especificada
    save(joinpath(scenario, "bomb_construction_bar_chart_energy_gwh.png"), fig_energy) #cambiado el nombre del archivo

    # Crear la figura y el eje con proporciones ajustadas para GenCapacity_GW
    fig_capacity = Figure(size=(1000, 600))
    ax_capacity = Axis(fig_capacity[1, 1], title="Capacidad construida de Almacenamiento PSP (GW) por Duración (Escenario: $(basename(scenario)))", xlabel="Periodo", ylabel="Capacidad de Generación (GW)",
        titlesize=24, xlabelsize=16, ylabelsize=16)
    ...
    # Guardar la figura en la ruta especificada
    save(joinpath(scenario, "bomb_construction_bar_chart_capacity_gw.png"), fig_capacity) #cambiado el nombre del archivo
end
