...
for scenario in scenarios
    ...
    filtered_df.storage_duration = [Int64(round(df2[df2.GENERATION_PROJECT .== row.generation_project, :gen_storage_energy_to_power_ratio][1])) for row in eachrow(filtered_df)] # Asegurar Int64
    ...
    function calculate_annual_increase(df::DataFrame, value_column::Symbol)
        ...
        for (i, current_period) in enumerate(unique_periods)
            ...
            for duration in unique(df.storage_duration)
                ...
                # Si es el primer período, el incremento es el valor actual
                if i > 1
    ...
    ax_energy = Axis(fig_energy[1, 1],
                    title = "Incremento Anual en Capacidad de Almacenamiento PSP (MWh) (Scenario: $(basename(scenario)))",
                    xlabel = "Periodo",
    ...
    ax_capacity = Axis(fig_capacity[1, 1],
                     title = "Incremento Anual en Capacidad Instalada de Almacenamiento PSP (MW) (Scenario: $(basename(scenario)))",
                     xlabel = "Periodo",
    ...
    if barplot_energy !== nothing
        Legend(fig_energy[1, 2],
               ...
               string.("Duración del Almacenamiento (Horas) : ", unique_durations_energy); # Agregado "Duración del Almacenamiento"
               labelcolor = :black,
               labelsize = 12,
               titlecolor = :black,
               titlefont = 10)
    ...
    if barplot_capacity !== nothing
        Legend(fig_capacity[1, 2],
               ...
               string.("Duración del Almacenamiento (Horas) : ", unique_durations_capacity); # Agregado "Duración del Almacenamiento"
               labelcolor = :black,
               labelsize = 12,
               titlecolor = :black,
               titlefont = 10)
    ...
    save(joinpath(scenario, "bomb_Construction_bar_chart_energy_increase.png"), fig_energy)
    save(joinpath(scenario, "bomb_Construction_bar_chart_installed_capacity_increase.png"), fig_capacity)
