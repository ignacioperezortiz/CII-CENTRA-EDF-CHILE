...
for scenario in scenarios
    ...
    pivot_df_energy = pivot_df_energy[!, [:"1", :"2", :"4", :"6"]]
    pivot_df_capacity = pivot_df_capacity[!, [:"1", :"2", :"4", :"6"]]

    rename!(pivot_df_energy, Dict(Symbol("1") => Symbol("1"), Symbol("2") => Symbol("2"), Symbol("4") => Symbol("4"), Symbol("6") => Symbol("6")))
    rename!(pivot_df_capacity, Dict(Symbol("1") => Symbol("1"), Symbol("2") => Symbol("2"), Symbol("4") => Symbol("4"), Symbol("6") => Symbol("6")))
    
    # Guardar las tablas pivotadas en nuevos archivos CSV
    CSV.write(joinpath(scenario, "ess_storage_capacity_energy.csv"), pivot_df_energy)
    CSV.write(joinpath(scenario, "ess_storage_capacity_mw.csv"), pivot_df_capacity)

    data_energy = pivot_df_energy
    data_capacity = pivot_df_capacity
    
    # Crear la figura y el eje con proporciones ajustadas para Storage_Energy_MWh
    fig_energy = Figure(size = (1400, 600))  # Aumentar el ancho de la figura
    ax_energy = Axis(fig_energy[1, 1], title = "Capacidad de Almacenamiento (MWh) por Duración (Scenario: $(basename(scenario)))", xlabel = "Periodo", ylabel = "Capacidad de Almacenamiento (MWh)",
    ...
    for i in 1:size(data_energy, 2)
        barplot!(ax_energy, period_energy .+ ((i - length(duration_order) / 2) * width) .+ (width / 2), data_energy[:, i], label = string(names(pivot_df_energy)[i]), color = colors[parse(Int, names(pivot_df_energy)[i])], width = width)
    end
    ...
    save(joinpath(scenario, "ess_bar_chart_storage_energy.png"), fig_energy)
    
    # Mostrar la figura
    display(fig_energy)
    
    # Crear la figura y el eje con proporciones ajustadas para GenCapacity_MW
    fig_capacity = Figure(size = (1400, 600))  # Aumentar el ancho de la figura
    ax_capacity = Axis(fig_capacity[1, 1], title = "Capacidad Instalada (MW) por Duración (Scenario: $(basename(scenario)))", xlabel = "Periodo", ylabel = "Capacidad Instalada (MW)",
                        titlesize = 24, xlabelsize = 16, ylabelsize = 16)
    
    # Crear el gráfico de barras para GenCapacity_MW
    for i in 1:size(data_capacity, 2)
        barplot!(ax_capacity, period_capacity .+ ((i - length(duration_order) / 2) * width) .+ (width / 2), data_capacity[:, i], label = string(names(pivot_df_capacity)[i]), color = colors[parse(Int, names(pivot_df_capacity)[i])], width = width)
    end
    ...
    # Guardar la figura en la ruta especificada
    save(joinpath(scenario, "ess_bar_chart_installed_capacity.png"), fig_capacity)
