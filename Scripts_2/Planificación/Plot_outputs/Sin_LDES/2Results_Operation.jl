...
for scenario in scenarios
    ...
    for period in unique_periods
        ...
        for day in unique_days
            ...
            ax = Axis(fig[1, 1], title="Dispatch Generation by Technology for $period-$day", xlabel="Hour of the Day", ylabel="Dispatch (MW)")
            for tech in reverse(names(pivot_df)[2:end])
                fill_between!(ax, pivot_df.hour, zeros(length(pivot_df.hour)), pivot_df[!, tech], label=tech, color=colors[tech])
            end
            ...
            # Plot the line with adjusted indices and as a dashed line
            lines!(ax, 0:23, line[1:24], linestyle=:dash, color=:black)
