...
base_dir = "C:/Users/Ignac/Trabajo_Centra/Catedra-LDES/CII-Centra-EDF/Estudio_Oficial/Sinsib/Sensibilidades/OK/Corridos/Ausencia_Diesel&GNL"
    ...
    for period in unique_periods
        ...
        for day in unique_days
            ...
            for tech in reverse(names(pivot_df)[2:end])
                fill_between!(ax, (pivot_df.hour.+1).*4, zeros(length(pivot_df.hour)), pivot_df[!, tech] ./ 1000, label=tech, color=colors[tech])
            end
            #
            ...
            hours_x = collect(4:4:96) # Create the x-axis values for the demand line
            lines!(ax, hours_x, line[1:24] ./ 1000, linestyle=:dash, color=:black, label="Demanda")
